...
async function clearLocalSession(reason, owner = null) {
  const update = {
    currentEmail: null,
    currentProject: null,
    sessionState: 'logged_out',
    lastLogoutReason: reason || 'ownership_lost'
  };

  if (owner?.userName) {
    update.lastSessionOwner = owner.userName;
    update.lastSessionOwnerProject = owner.projectPath || 'dashboard';
  } else {
    update.lastSessionOwner = null;
    update.lastSessionOwnerProject = null;
  }
 ...
 */
async function sendSessionUpdate(action, email, project, force = false, reason = null) {
  try {
    ...
    if (data.success && data.ownershipLost) {
      console.warn('[Webflow Tracker] Session ownership lost:', data.currentOwner);
      await clearLocalSession('ownership_lost', data.currentOwner);
      return data;
    }
  ...
  switch (message.type) {
    ...
    case 'LOGOUT_CLICKED':
      ...
      if (message.email) {
        ...
        sendSessionUpdate('release', message.email, null, true, message.reason || 'manual');
        clearLocalSession(message.reason || 'manual');
      }
      break;
  }
