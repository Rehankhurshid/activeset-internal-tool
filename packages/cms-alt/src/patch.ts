...
import type { CmsUpdatePayload } from './types';
  ...
  for (const [itemKey, fields] of byItemField) {
    ...
    for (const [fieldSlug, bucket] of fields) {
      if (bucket.type === 'RichText') {
        ...
        // Each buildMultiImageFieldPatch rebuilds from the ORIGINAL array, so
        // if two payloads target the same field, the second clobbers the first.
        // Apply all updates at once to a single working array.
