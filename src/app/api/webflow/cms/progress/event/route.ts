/**
 * Append a progress event for an existing run. Called by the local CLI.
 *
 * Auth: the CLI passes `{ runId, secret }` — the matching pair was issued
 * by /progress/start. Firestore rules are bypassed because this uses the
 * admin SDK; request validation is the whole security boundary.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!runId || !secret || !event || !event.step || !event.message) {
      return NextResponse.json({ error: 'Missing runId, secret, or event fields' }, { status: 400 });
    }
    ...
    const run = runSnap.data() as { secret?: string; expiresAt?: admin.firestore.Timestamp };
