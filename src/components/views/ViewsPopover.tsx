...
/** One row from a `/views` endpoint (proposals and client portals share it). */
...
interface ViewsPopoverProps {
    /** Route returning `{ views: ViewRow[] }`; `?limit=50` is appended here. */
    endpoint: string;
    viewCount: number;
    /** Second line of the popover header, e.g. "Public share link · viewer identity not captured". */
    subtitle: string;
    children: ReactNode;
}
...
async function fetchViews(endpoint: string): Promise<ViewRow[]> {
    ...
    const json = (await res.json()) as { views: ViewRow[] };
    return json.views || [];
}

/**
 * "Opened N times" popover listing recent opens of a public link. Renders the
 * children as-is when there are no views yet (nothing to open), otherwise wraps
 * them in a trigger and lazy-loads the rows the first time it opens.
 */
export function ViewsPopover({ endpoint, viewCount, subtitle, children }: ViewsPopoverProps) {
