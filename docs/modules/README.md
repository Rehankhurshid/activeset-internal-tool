...
| [project-links.md](./project-links.md) | **Client Projects**, the hub. Project list, the New project dialog, the project detail page and **its tab table** (which module owns each tab), manual links, the `projects` doc (every field), embeddable `widget.js`, legacy audit share link, daily review loop and evening digest | `/modules/project-links`, `/modules/project-links/[id]`, `/embed`, `/share/project-links/[token]` | `src/modules/project-links`, `src/services/database.ts`, `src/types/index.ts` |
