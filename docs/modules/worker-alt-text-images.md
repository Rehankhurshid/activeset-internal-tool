...
| Every page with the app navigation bar → **worker indicator** beside Scans | [WorkerActivityIndicator.tsx](../../src/components/navigation/WorkerActivityIndicator.tsx) (mounted in [AppNavigation.tsx](../../src/shared/ui/AppNavigation.tsx)) | Live from `worker_jobs` (status `queued`/`running`, all projects): the running job's project, group, item and field (`currentLabel`), reading/shrinking with the animated image, progress; queued jobs in claim order (priority, then age) with cancel and Cancel all; "Open" → `?tab=webflow&section=images`. On a phone: just the image on the bar, a strip under the bar with the item and %, full-width panel; hidden when idle | Signed-in team. Cancel is a client transaction that only succeeds on a job still `queued` |
...
- [handlers/library-sweep.ts](../../src/lib/worker/handlers/library-sweep.ts) — `runLibrarySweep` (job `library_sweep`): the hourly "Auto-optimise new images" pass over every group with `newOnly`.
...
- Webflow helpers used by handlers (owned by the Webflow/CMS doc): `src/lib/cms/library.ts` (`listSiteAssets`, `listCollectionImages`, `buildCmsIndex`), `src/lib/cms/placement.ts` (`placementsFor`, `ensureOptimisedFolder`, `OPTIMISED_FOLDER_NAME = 'ActiveSet · optimised'`), `src/lib/cms/webflow-client.ts` (`webflowFetch` retries 429 and, for non-POST calls and publish, dropped connections, `patchItems`, `publishItems`), `src/lib/cms/assets.ts` (`uploadAssetToWebflow`, `sanitizeAssetFileName`), `src/lib/cms/patch.ts` (`groupUpdatesByItem`), `src/lib/project-admin.ts` (`loadProjectDocAdmin`, `getWebflowTokenAdmin`).
...
- [src/components/navigation/WorkerActivityIndicator.tsx](../../src/components/navigation/WorkerActivityIndicator.tsx) — `WorkerActivityIndicator` (subscribes via `workerRepository.subscribeActiveJobs`, cancels via `cancelQueued`) and `WorkerActivityView` (pure display, props only).
- [src/components/webflow/WorkingThumb.tsx](../../src/components/webflow/WorkingThumb.tsx) — the animated thumbnail (scan line while reading, squeeze while shrinking; `animate-image-*` in globals.css, off under reduced motion).
...
| `library_group` | Images screen Optimise / Optimise everything / selected rows | `LibraryGroupPayload` ([library-group.ts](../../src/lib/worker/handlers/library-group.ts)): `group` (`{kind:'assets'}` or `{kind:'collection', collectionId, name?}`), `srcs?`, `steps {alt?, images?}` (both default on), `force`, `publish`, `by`, `newOnly` (only images with no draft and no index `optimise` entry; set by `library_sweep`) | Lists the group's images (General assets = image assets minus `cmsSourceAssetIds`); ALT half: redraft missing images without a confident draft, one image at a time (saved immediately), then `runAltApply` for confident ones (`!needsReview && certainty !== 'low'`), record ALT state in index; images half: `runImageApply` with `designerCopies` for assets. Throws only if neither half did anything and there are errors | Everything the two sub-handlers write + `image_index.alt` for every image in the group |
| `library_sweep` | Hourly cron `/api/cron/auto-optimise` for projects with `autoOptimiseImages.enabled`; fixed job id `auto-images-<projectId>` (`ensureJob` replaces a finished one, skips a queued/running one), priority -1 | `LibrarySweepPayload` ([library-sweep.ts](../../src/lib/worker/handlers/library-sweep.ts)): `by` | Returns early if the switch was turned off; otherwise `runLibraryGroup` with `newOnly: true, publish: false` for General assets then every collection (`listCollections`), progress label prefixed with the group name; a failing group is recorded and the sweep continues | Same as `library_group`; result `{groups:[{name, images, altAdded, altHeld, optimised, errors}], changed}` read by the Images screen switch |
