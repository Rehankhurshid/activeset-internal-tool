...
| Change the New project dialog (name, client, project type → checklist, dates, engagement, lead, client contacts, client plan) | [NewProjectDialog.tsx](../../src/modules/project-links/ui/components/NewProjectDialog.tsx); fields written by `newProjectFields` / `createProject` in [database.ts](../../src/services/database.ts) |
...
- [ui/components/NewProjectDialog.tsx](../../src/modules/project-links/ui/components/NewProjectDialog.tsx): the New project dialog. Only the name is required. Project type lists the SOP templates (built-ins at once, custom ones from `checklistService.getSOPTemplates()` when that read succeeds; "No checklist for now" is the default). Start and target end dates, engagement tag, lead (defaults to you; becomes `reviewOwnerEmail` and the first assignee), client contacts (`clientPortal.contactEmails`, portal stays off), "Set up the client dashboard" (on by default) with a live preview of the stages. Create: `createProject(uid, name, setup)` with the plan from `draftClientPlan([AGENCY_START, ...template.sections, AGENCY_CLOSE], dates)`, then `checklistService.createChecklist(id, [templateId])` (a failure there warns and still creates the project). The success toast's "Open" goes to the Client tab. While it is open the dashboard's single-key shortcuts (`n`, `/`, `v`, `c`, `j`/`k`, Enter) are paused, so Enter on its buttons cannot open a hovered card.
...
|---|---|---|
| `createProject(userId, name, setup?)` | add `projects` doc: `name, userId, status:'current', tags:[], links:getDefaultLinks(), createdAt, updatedAt`, plus from `setup` (`NewProjectSetup`) only what is filled in: `client`, `tags`, `reviewOwnerEmail`, `assigneeEmails` (the lead first), `clientPortal: { enabled: false, contactEmails }`, `clientPlan` | Default links: Project Tracker, Staging Website URL, Live Website URL, Feedback URL (empty URLs, `isDefault: true`, no `source`). Empty setup fields are left off (Firestore refuses `undefined`). |
...
| `updateClientFacing(id, patch, byEmail, {touch?})`, `markClientUpdated`, `updateClientPortalSettings`, `updateClientPlan(id, mutate, byEmail)` (transaction) | `clientFacing.*`, `clientPortal.*`, `clientPlan` | Client-portal writes; see [client-portal.md](./client-portal.md). |
...
| `clientPortal`, `clientFacing`, `clientPlan` | portal settings/status/counters; the client dashboard's plan (stages, dates, what they get, files) | [client-portal.md](./client-portal.md); the plan is first written by the New project dialog |
...
1. **Create a project**: `n`, "New Project" or `?new=1` → `NewProjectDialog` → `projectLinksRepository.createProject(user.uid, name, setup)` → `projectsService.createProject` (`addDoc` with 4 default links, status `current`, and the dialog's client, tag, lead, contacts and drafted client plan) → `checklistService.createChecklist` when a project type was picked → `handleProjectCreated`: filter switches to "All" and tag chips clear so the project is visible, and the toast offers "Open" (the Client tab) → `subscribeToAllProjects` listener delivers it.
...
- **Keep `grid-cols-1` on the dashboard's card grids** (ProjectLinksDashboardScreen.tsx, both the plain and the By-Client grid). It looks redundant but is not. Below `md` there is otherwise no column template, so the grid's single implicit `auto` column grows to the widest card's min-content, and a `truncate`d name's min-content is the whole name. One long project name then made every card wider than a phone (609 px cards in a 366 px container at a 390 px viewport), with the "…" menu and people chip off-screen. `grid-cols-1` is `repeat(1, minmax(0, 1fr))`, which caps the column at the container width.
