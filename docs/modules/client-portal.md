...
- **`updateClientFacing` only stamps freshness with `{ touch: true }`** ([database.ts:1231-1250](../../src/services/database.ts)). `lastUpdateAt` is the sole input to the stale nudge; the project card's status change deliberately omits `touch` ([ProjectCard.tsx:143-152](../../src/components/projects/ProjectCard.tsx)); Client tab Save passes it ([ClientStatusEditor.tsx:106-108](../../src/modules/client-portal/ui/components/ClientStatusEditor.tsx)).
