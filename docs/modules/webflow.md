...
- [WebflowImagesDashboard.tsx](../../src/components/webflow/WebflowImagesDashboard.tsx) — `WebflowImagesDashboard`: one section per group (General assets, then each CMS collection), per-section Optimise + "Optimise everything", publish toggle, per-row draft/Save, `LiveBadge` (live / pending / never) comparing staged ALT with `cms/live`. On a phone the section header takes the full width (counts wrap under the name, Optimise drops below) and each row's badges wrap. No browser-side Webflow writes.
