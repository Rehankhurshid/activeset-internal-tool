...
- [WebflowPagesDashboard.tsx](../../src/components/webflow/WebflowPagesDashboard.tsx) — `WebflowPagesDashboard`: the whole Webflow tab. Pages table (filters All/Has Issues/Critical/Good/Static/CMS Templates; sorts; "show drafts" switch; locale select), per-row draft/archive toggles, Copy DOM Details, View page (link built from `customDomain` or a `siteName`-derived `*.webflow.io` guess), hosts the Images / Schema / Sitemap Sync sub-tabs.
